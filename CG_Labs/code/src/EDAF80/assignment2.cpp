...
edaf80::Assignment2::run()
{
	...
	auto const shape = parametric_shapes::createSphere(60u, 60u, 2.0f);
	//auto const shape = parametric_shapes::createTorus(60u, 60u, 3.0f, 1.0f);
	//auto const shape = parametric_shapes::createQuad(2.0f, 2.0f);
	...
	std::vector<glm::vec3> cp{ p0, p1, p2, p3, p4, p5, p6, p7, p8, p9 };

	float path_pos = 0;
	int i = 0;
	float pos_velocity = 0.01;
	...
	while (!glfwWindowShouldClose(window)) {
		...
		if (use_linear) {
			glm::vec3 p0 = cp[i];
			glm::vec3 p1 = cp[(i + 1) % N];

			glm::vec3 p = interpolation::evalLERP(p0, p1, path_pos);
			circle_rings.set_translation(p);
			...
			glm::vec3 p0 = cp[i];
			glm::vec3 p1 = cp[(i + 1) % N];
			glm::vec3 p2 = cp[(i + 2) % N];
			glm::vec3 p3 = cp[(i + 3) % N];

			glm::vec3 p = interpolation::evalCatmullRom(p0, p1, p2, p3, catmull_rom_tension, path_pos);
			circle_rings.set_translation(p);
