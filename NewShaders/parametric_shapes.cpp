...
parametric_shapes::createQuad(unsigned int width, unsigned int height)
{
	...
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, /*! \todo how many bytes should the buffer contain? */static_cast<GLsizeiptr>(indices.size() * sizeof(glm::uvec3)),
	             /* where is the data stored on the CPU? */reinterpret_cast<GLvoid const*>(indices.data()),
	             /* inform OpenGL that the data is modified once, but used often */GL_STATIC_DRAW);
...
bonobo::mesh_data
parametric_shapes::createWaterQuad(unsigned int width, unsigned int height, unsigned int res_width, unsigned int res_height)
{
	...
	for (unsigned int i = 0u; i <= res_width; ++i) {
		float width_pos = float(i) / res_width * width;

		for (unsigned int j = 0u; j <= res_height; ++j) {
			float height_pos = float(j) / res_height * height;

			// vertex coordinates
			vertices[index] = glm::vec3(width_pos,
				0,
				height_pos);

			// texture coordinates
			texcoords[index] = glm::vec3(width_pos/width,
				height_pos / height,
				0);
	...
	for (unsigned int i = 0u; i < res_width; ++i)
	{
		for (unsigned int j = 0u; j < res_height; ++j)
		{
			indices[index] = glm::uvec3((res_width + 1) * i + j,
				(res_width + 1) * i + j + (res_height+1),
				(res_width + 1) * i + j + 1u + (res_height + 1)
				);
			++index;
                                ...
                                unsigned int const res_phi, float const radius)
{
  ...
  // generate vertices iteratively
	...
	for (unsigned int i = 0u; i < res_theta - 1u; ++i)
	{
		for (unsigned int j = 0u; j < res_phi - 1u; ++j)
		{
			indices[index] = glm::uvec3(res_phi * i + j,
				res_phi * i + j + 1u + res_theta,
				res_phi * i + j + 1u);
			++index;

			indices[index] = glm::uvec3(res_phi * i + j,
				res_phi * i + j + res_theta,
				res_phi * i + j + 1u + res_theta);
			++index;
		}
	}
	...
	auto const radius_offset = binormals_offset + binormals_size;
	auto const radius_size = static_cast<GLsizeiptr>(1 * sizeof(float));
	auto const bo_size = static_cast<GLsizeiptr>(vertices_size
		+ normals_size
		+ texcoords_size
		+ tangents_size
	...
	glBufferSubData(GL_ARRAY_BUFFER, radius_offset, radius_size, &radius);
  ...
  // generate vertices iteratively
	...
	for (unsigned int i = 0u; i < res_theta; ++i) {
		...
		for (unsigned int j = 0u; j < res_phi; ++j) {
			...
			// binormal
			auto b = glm::vec3(-(rA + rB * cos_theta)*sin_phi, (rA + rB * cos_theta)*cos_phi, 0.0f);
			b = glm::normalize(b);
			binormals[index] = b;
                                    ...
                                    float const outer_radius)
{
	...
	// generate indices iteratively WHY DOES NOT DEPEND THIS ON THETA?
	index = 0u;
	for (unsigned int i = 0u; i < res_theta - 1u; ++i)
	{
